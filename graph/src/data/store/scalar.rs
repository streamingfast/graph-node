...
mod test {
    ...
    #[test]
    fn big_decimal_checks() {
        let dec = |x: &str| (x.to_string(), BigDecimal::from_str(x).unwrap());

        #[rustfmt::skip]
        let cases: Vec<(_, &str, i64)> = vec![
            (dec("0.1"), "1", 1),
            // (dec("-0.1"), "-1", 1),
            // (dec("198.98765544"), "19898765544", 8),
            // (dec("0.00000093937698"), "93937698", 14),
            // (dec("98765587998098786876.0"), "98765587998098786876", 0),
            // (dec("98765000000"), "98765", -6),
            // (dec("-98765000000"), "-98765", -6),
            // (dec("98765000000.1"), "987650000001", 1),
            // (dec("-98765000000.2"), "-987650000002", 1),

            // // Positive rounding outside max scale (34)
            // (dec("0.1234567890123456789012345678901234"), "1234567890123456789012345678901234",34),
            // (dec("0.12345678901234567890123456789012344"), "1234567890123456789012345678901234", 34),
            // (dec("0.12345678901234567890123456789012345"), "1234567890123456789012345678901235", 34),
            // (dec("0.12345678901234567890123456789012346"), "1234567890123456789012345678901235", 34),

            // // Negative rounding outside max scale (34)
            // (dec("-0.1234567890123456789012345678901234"), "-1234567890123456789012345678901234", 34),
            // (dec("-0.12345678901234567890123456789012344"), "-12345678901234567890123456789012344", 35),
            // (dec("-0.12345678901234567890123456789012345"), "-12345678901234567890123456789012345", 35),
            // (dec("-0.12345678901234567890123456789012346"), "-12345678901234567890123456789012346", 35),

            // // Normalize negative numbers have a bug where scale is actually MAX + 1
            // (dec("-0.123456789012345678901234567890123424"), "-12345678901234567890123456789012342", 35),
            // (dec("-0.123456789012345678901234567890123425"), "-12345678901234567890123456789012342", 35),
            // (dec("-0.123456789012345678901234567890123426"), "-12345678901234567890123456789012342", 35),

            // // Showcasing rounding effects when max digits is split before/after dot
            // (dec("12.123456789012345678901234567890124"), "1212345678901234567890123456789012", 32),
            // (dec("12.123456789012345678901234567890125"), "1212345678901234567890123456789013", 32),
            // (dec("12.123456789012345678901234567890126"), "1212345678901234567890123456789013", 32),

            // (dec("-12.1234567890123456789012345678901234"), "-12123456789012345678901234567890123", 33),
            // (dec("-12.1234567890123456789012345678901235"), "-12123456789012345678901234567890123", 33),
            // (dec("-12.1234567890123456789012345678901236"), "-12123456789012345678901234567890123", 33),

            // (dec("1234567890123.123456789012345678901834567890124"), "1234567890123123456789012345678902", 21),
            // (dec("-1234567890123.123456789012345678901894567890124"), "-12345678901231234567890123456789018", 22),

            // // Showcasing rounding effects when max digits is all before dot
            // (dec("1234567890123456789012345678901234"), "1234567890123456789012345678901234", 0),
            // (dec("12345678901234567890123456789012344"), "1234567890123456789012345678901234", -1),
            // (dec("12345678901234567890123456789012345"), "1234567890123456789012345678901235", -1),
            // (dec("12345678901234567890123456789012346"), "1234567890123456789012345678901235", -1),

            // (dec("-12345678901234567890123456789012345"), "-12345678901234567890123456789012345", 0),
            // (dec("-123456789012345678901234567890123454"), "-12345678901234567890123456789012345", -1),
            // (dec("-123456789012345678901234567890123455"), "-12345678901234567890123456789012345", -1),
            // (dec("-123456789012345678901234567890123456"), "-12345678901234567890123456789012345", -1),

            // (dec("10000000000000000000000000000000000000000"), "1", -40),
            // (dec("100000000000000000000000000000000000000001"), "1", -41),

            // (dec("19999999999999999999999999999999994"), "1999999999999999999999999999999999", -1),
            // (dec("19999999999999999999999999999999995"), "2", -34),
            // (dec("19999999999999999999999999999999985"), "1999999999999999999999999999999999", -1),

            // (dec("1999999999999999999999999999999999"), "1999999999999999999999999999999999", 0),
            // (dec("199999999999999999999999999999999"), "199999999999999999999999999999999", 0),
            // (dec("19999999999999999999999999999999999"), "2", -34),
            // (dec("199999999999999999999999999999999999999999"), "2", -41),

            // (dec("1444444444444444444444444444444444"), "1444444444444444444444444444444444", 0),
            // (dec("14444444444444444444444444444444444"), "1444444444444444444444444444444444", -1),
            // (dec("144444444444444444444444444444444444"), "1444444444444444444444444444444444", -2),

            // (dec("1555555555555555555555555555555555"), "1555555555555555555555555555555555", 0),
            // (dec("15555555555555555555555555555555555"), "1555555555555555555555555555555556", -1),
            // (dec("155555555555555555555555555555555555"), "1555555555555555555555555555555556", -2),
        ];

        for ((input, dec), digits, scale) in cases.into_iter() {
            let (bigint, exp) = dec.as_bigint_and_exponent();

            let other_dec = bigdecimal::BigDecimal::from_str(&input).unwrap();
            let (other_bigint, other_exp) = other_dec.as_bigint_and_exponent();

            assert!(
                BigInt::from_str(digits).unwrap().0 == other_bigint && other_exp == scale,
                "for plain {} (big int [actual {}, expected {}], scale [actual {}, expected {}])",
                input,
                other_bigint,
                BigInt::from_str(digits).unwrap().0,
                other_exp,
                scale
            );

            assert!(
                BigInt::from_str(digits).unwrap().0 == bigint && exp == scale,
                "for {} (big int [actual {}, expected {}], scale [actual {}, expected {}])",
                input,
                bigint,
                BigInt::from_str(digits).unwrap().0,
                exp,
